...
void RayTracing::CreateRaytracingPipelineState(std::wstring raytracingShaderLibraryFile)
{
	...
	// There are 8 subobjects that make up our raytracing pipeline object:
	...
	rootSigAssociation.pSubobjectToAssociate = &subobjects[4]; // Root sig above
...
void RayTracing::CreateEntityDataBuffer(std::vector<std::shared_ptr<Entity>> sceneEntities) 
{
	// For every entity, store the index of the index buffer VIEW, the vertex buffer VIEW, and the color it'll be tinted. 
	// All these sets of data are then stored in one long buffer
	...
	// Recreate the buffer -> Sized exactly to the number of entities in the scene
	UINT64 bufferSize = sizeof(EntityData) * entityData.size();

	EntityDataStructuredBuffer.Reset(); // Resetting ?
	...
	// If the buffer hasn't been tracked yet, a slot needs to be reserved ?
	...
	uavDesc.Buffer.StructureByteStride = sizeof(EntityData);
...
// --------------------------------------------------------
void RayTracing::CreateTopLevelAccelerationStructureForScene(std::vector<std::shared_ptr<Entity>> sceneEntities)
{
	...
	for (int i = 0; i < sceneEntities.size(); i++) 
	{
		...
		DirectX::XMFLOAT4X4 transform = sceneEntities[i]->GetTransform()->GetWorldMatrix();
	...
	if (sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * sceneEntities.size() > tlasInstanceDataSizeInBytes[frameIndex])
	{
		// Reset and save the new size
		TLASInstanceDescBuffer[frameIndex].Reset();
		tlasInstanceDataSizeInBytes[frameIndex] = sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * sceneEntities.size();
	...
	memcpy(mapped, &instanceDescs[0], sizeof(D3D12_RAYTRACING_INSTANCE_DESC));
...
void RayTracing::Raytrace(std::shared_ptr<Camera> camera, Microsoft::WRL::ComPtr<ID3D12Resource> currentBackBuffer)
{
	...
	// ACTUAL RAYTRACING HERE
	{
		...
		data.EntityDataDescriptorIndex = Graphics::GetDescriptorIndex(cbuffer);
		data.SceneDataCBIndex = Graphics::GetDescriptorIndex(EntityDataUAV_GPU);
